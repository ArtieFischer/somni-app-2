-- Apply Dream Sharing Migration (Correct Version)
-- Based on supabase-clean schema

-- First, check if shared_dreams already exists
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'shared_dreams') THEN
...
DROP TRIGGER IF EXISTS update_shared_dreams_updated_at ON shared_dreams;
...
-- Add to realtime subscriptions for live updates (if not already added)
DO $$
BEGIN
...
GRANT SELECT ON shared_dreams TO anon, authenticated;
...
SELECT 'Dream sharing migration completed successfully!' as status;