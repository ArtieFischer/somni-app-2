...
const getAnalyzedByText = (interpreterId: string) => {
  const names = {
    jung: 'Analyzed by Dr. Carl',
    freud: 'Analyzed by Dr. Sigmund',
    lakshmi: 'Analyzed by Lakshmi',
    mary: 'Analyzed by Mary',
  };
  return names[interpreterId] || 'Analyzed';
};
...
export const InterpretationDisplay: React.FC<InterpretationDisplayProps> = ({
  ...
  const interpretationData = isApiResponse
    ? (interpretation as InterpretationResponse)
    : null;
  const dbInterpretation = !isApiResponse
          ...
          >
            <HStack space="sm" style={{ alignItems: 'center' }}>
              <MaterialCommunityIcons
                name={getInterpreterIcon(
                  interpretation?.interpreter_type ||
                    interpretation?.interpreter_id ||
                    interpretationData?.interpreterId ||
                    '',
                )}
                size={20}
            ...
            {dbInterpretation?.created_at && (
              <Text
                style={{ fontSize: 12, color: darkTheme.colors.text.secondary }}
              >
            ...
            {fullResponse?.advice ? (
              ...
              <VStack space="sm">
                {interpretationData.practicalGuidance.map((guidance, i) => (
                  ...
                  >
                    <Text
                      style={{ fontSize: 14, color: darkTheme.colors.primary }}
                    >
                      •
                    </Text>
                    <Text
