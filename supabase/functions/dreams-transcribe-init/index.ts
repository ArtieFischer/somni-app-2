...
serve(async (req) => {
  ...
  try {
    ...
    const { dreamId, audioBase64, duration, language, locationMetadata }: TranscribeRequest = body;
    ...
    console.log('Request data:', {
      ...
      language,
      hasLocationMetadata: !!locationMetadata
    });
    ...
    const { data: dream, error: dreamError } = await supabase.from('dreams').update({
      transcription_status: 'processing'
    }).eq('id', dreamId).eq('user_id', user.id).select('id, raw_transcript').single();
    ...
    try {
      ...
      console.log('📦 Backend request payload:', {
        ...
        locationMetadata: backendPayload.locationMetadata
      });
      ...
      const { error: saveError } = await supabase.from('dreams').update({
      ...
      if (duration) {
        console.log('📊 Recording transcription usage...', {
          dreamId,
          duration,
          userId: user.id
        });
